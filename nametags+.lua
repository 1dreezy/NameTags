            ...
            if v ~= 1 then
                ...
                if isCharOnScreen(v) and wallPlayer(v) and not sampIsPlayerNpc(id) and res and sampIsPlayerConnected(id) then
                    ...
                    if config.showHealth then
                        ...
                        local healthTextPosX = ix + 10  -- Mai la stânga cu 2 pixeli
                        local healthTextPosY = iy - 8   -- Mai sus cu 2 pixeli
                        if config.background then
                            for dx = -1, 1 do
                                for dy = -1, 1 do
                                    if dx ~= 0 or dy ~= 0 then
                    ...
                    if armor > 0 then
                        ...
                        local armorTextPosX = cx + 10  -- Mai la stânga cu 2 pixeli
                        local armorTextPosY = cy - 8   -- Mai sus cu 2 pixeli
                        if config.background then
                            for dx = -1, 1 do
                                for dy = -1, 1 do
                                    if dx ~= 0 or dy ~= 0 then
...
if sampIsPlayerPaused(id) and config.showAfk then
    lua_thread.create(function ()
        local offsetX = -7.0   -- Mută mai mult spre stânga
        local offsetY = -4.5   -- Mută mai sus
        local scaleX = 1.80  -- Lungește imaginea pe orizontală (ajustează dacă vrei mai mult sau mai puțin)
        local scaleY = 1.05  -- Mărirea pe verticală

        -- Ajustări pentru mărime și poziție
        dl:AddImage(
            afkpng, 
    ...
    function(player)
        ...
        if imgui.Begin('NameTags+', renderWindow, imgui.WindowFlags.NoResize) then
            ...
            if imgui.Button('Reset') then
             --   local pStSet = sampGetServerSettingsPtr() + 39
               local dist = memory.getfloat(sampGetServerSettingsPtr() + 39)
                 slider[0] = dist
                config.distance = dist
            end
            imgui.Text('Author: tinyurl.com/wadexc')
            if imgui.IsItemClicked() then os.execute('explorer https://tinyurl.com/wadexc') end
            imgui.Text('Made for Visuals+')
            if imgui.IsItemClicked() then os.execute('explorer https://tinyurl.com/visualsproject') end
            imgui.PushFont(nunitoFont)
            imgui.PopFont()
            imgui.End()
        end
...
function Theme()
    ...
    -- Culori personalizate (text alb + negru foarte transparent + mov lila vizibil)
    style().Colors[imgui.Col.Text] = imgui.ImVec4(1, 1, 1, 1)  -- Text alb
    style().Colors[imgui.Col.WindowBg] = imgui.ImVec4(0, 0, 0, 0.4)  -- Negru foarte transparent
